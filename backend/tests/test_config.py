...
class TestDatabaseConfig:
    ...
    def test_default_values(self):
        ...
        assert config.db_path == str(Path.home() / ".trackai" / "trackai.duckdb")
        assert config.s3_bucket is None
        assert config.s3_key == "trackai.duckdb"
...
class TestTrackAIConfig:
    """Tests for TrackAIConfig model."""

    def test_default_database_config(self):
        """Test that TrackAIConfig has default database config."""

        config = TrackAIConfig()
        assert isinstance(config.database, DatabaseConfig)
        assert "database" in TrackAIConfig.model_fields
...
class TestConfigFileOperations:
    ...
    def test_save_and_load_config(self, temp_config_dir, monkeypatch):
        ...
        # Clear TRACKAI_DB_PATH to avoid interference from other tests
        monkeypatch.delenv("TRACKAI_DB_PATH", raising=False)
        ...
        monkeypatch.setattr(
            "trackai.config.CONFIG_FILE", temp_config_dir / "config.json"
        )
    ...
    def test_load_config_nonexistent_file(self, temp_config_dir, monkeypatch):
        ...
        monkeypatch.setattr(
            "trackai.config.CONFIG_FILE", temp_config_dir / "nonexistent.json"
        )
    ...
    def test_load_config_invalid_json(self, temp_config_dir, monkeypatch):
        ...
        # Patch CONFIG_FILE
        monkeypatch.setattr("trackai.config.CONFIG_FILE", invalid_file)
...
class TestEnvironmentVariables:
    ...
    def test_env_var_override_db_path(self, monkeypatch):
        """Test that TRACKAI_DB_PATH environment variable overrides config."""

        # Set environment variable
        monkeypatch.setenv("TRACKAI_DB_PATH", "/env/db.duckdb")

        config = load_config()
        assert config.database.db_path == "/env/db.duckdb"

    def test_env_var_override_s3_bucket(self, monkeypatch):
        """Test that TRACKAI_S3_BUCKET environment variable overrides config."""

        # Set environment variable
        monkeypatch.setenv("TRACKAI_S3_BUCKET", "env-bucket")

        config = load_config()
        assert config.database.s3_bucket == "env-bucket"

    def test_env_var_override_s3_key(self, monkeypatch):
        """Test that TRACKAI_S3_KEY environment variable overrides config."""

        # Set environment variable
        monkeypatch.setenv("TRACKAI_S3_KEY", "env-key.duckdb")

        config = load_config()
        assert config.database.s3_key == "env-key.duckdb"

    def test_env_var_override_s3_region(self, monkeypatch):
        """Test that TRACKAI_S3_REGION environment variable overrides config."""

        # Set environment variable
        monkeypatch.setenv("TRACKAI_S3_REGION", "eu-west-1")
